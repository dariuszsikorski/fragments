...
async function ensureCleanDirectories(outputDir = './react-learn') {
...
async function main() {
  Logger.info('Starting React Learn Documentation Scraper - Full Pipeline');
  ...
  try {
    ...
    await ensureCleanDirectories('./react-learn');
    Logger.success('Cleanup completed successfully');
  } catch (error) {
    Logger.error(`Cleanup failed: ${error.message}`);
    process.exit(1);
  ...
  const phases = [
    {
      ...
      script: path.join(SCRIPTS_DIR, '01-extract-links-learn.js'),
      description: 'Scraping sidebar navigation and extracting Learn section links'
    },
    {
      number: 2, 
      name: 'Download HTML Pages',
      script: path.join(SCRIPTS_DIR, '02-download-html-learn.js'),
      description: 'Downloading original HTML content from all Learn pages (20 concurrent)'
    },
    {
      number: 3,
      name: 'Convert to Markdown',
      script: path.join(SCRIPTS_DIR, '03-convert-markdown-learn.js'), 
  ...
  try {
    ...
    console.log('\nCheck: ./react-learn/markdown/ for generated documentation');
    console.log('Open: ./react-learn/markdown/00-0-index-of-contents.md for complete navigation');
